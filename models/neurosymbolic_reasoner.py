...
class SymbolicReasoner:
    ...
    def __init__(self, kg_dir: str, rules_path: Optional[str] = None):
        self.kg_dir = Path(kg_dir)
        self.rules_path = Path(rules_path) if rules_path else None
        self.load_knowledge_graphs()
        
    def load_knowledge_graphs(self):
        """Load pre-built knowledge graphs or expanded rules with provenance."""
        # Patient and care-pathway graphs remain optional for symbolic checks
        try:
            with open(self.kg_dir / "patient_kg.pkl", 'rb') as f:
                self.patient_kg = pickle.load(f)
        except Exception:
            self.patient_kg = None
        
        try:
            with open(self.kg_dir / "care_pathway_kg.pkl", 'rb') as f:
                self.care_pathway_kg = pickle.load(f)
            self.has_care_pathways = True
        except Exception:
            self.care_pathway_kg = None
            self.has_care_pathways = False

        if self.rules_path and self.rules_path.exists():
            self.clinical_rules_kg = self._load_rules_from_json(self.rules_path)
        else:
            with open(self.kg_dir / "clinical_rules_kg.pkl", 'rb') as f:
                self.clinical_rules_kg = pickle.load(f)

        # Pre-compute contraindication pairs for fast lookup
        self.contra_pairs: List[Dict[str, str]] = []
        self.interaction_pairs: List[Dict[str, str]] = []
        for u, v, data in self.clinical_rules_kg.edges(data=True):
            if data.get('edge_type') == 'CONTRAINDICATION':
                med = None
                cond = None
                # Identify medication and condition endpoints regardless of direction
                if str(u).startswith(("medication", "medication_class")):
                    med = str(u).split(":", 1)[1]
                if str(v).startswith(("medication", "medication_class")):
                    med = med or str(v).split(":", 1)[1]
                if str(u).startswith("condition"):
                    cond = str(u).split(":", 1)[1]
                if str(v).startswith("condition"):
                    cond = cond or str(v).split(":", 1)[1]
                if med and cond:
                    self.contra_pairs.append({"med": med, "condition": cond, **data})
                elif str(u).startswith(("medication", "medication_class")) and str(v).startswith(("medication", "medication_class")):
                    self.interaction_pairs.append({
                        "med_a": str(u).split(":", 1)[1],
                        "med_b": str(v).split(":", 1)[1],
                        **data,
                    })

        print("Loaded knowledge graphs and clinical rules")

    def _load_rules_from_json(self, path: Path):
        """Load clinical rules from JSON (expanded, provenance-tracked)."""
        import json
        import networkx as nx
    ...
    def check_contraindications(
        ...
        violations: List[str] = []
        meds = {m.lower() for m in context.medications}
        conditions = {c.lower() for c in context.conditions}

        # Medication-condition contraindications
        for edge in self.contra_pairs:
            med = edge.get("med", "").lower()
            cond = edge.get("condition", "").lower()
            if med and cond and med in meds and cond in conditions:
                msg = (
                    f"Contraindication: {med} with {cond} "
                    f"(severity={edge.get('severity', 'unknown')}, "
                    f"provenance={edge.get('provenance', 'unspecified')})"
                )
                violations.append(msg)

        # Medication-medication interactions
        for edge in self.interaction_pairs:
            a = edge.get("med_a", "").lower()
            b = edge.get("med_b", "").lower()
            if a in meds and b in meds:
                msg = (
                    f"Interaction: {a} with {b} "
                    f"(severity={edge.get('severity', 'unknown')}, "
                    f"provenance={edge.get('provenance', 'unspecified')})"
                )
                violations.append(msg)
    ...
    def find_successful_pathways(
        ...
        if self.has_care_pathways and last_intervention in self.care_pathway_kg:
...
def load_neurosymbolic_reasoner(
    ...
    rl_checkpoint: str = str(Path(__file__).resolve().parent.parent / "trained_models" / "best_model.pt")
